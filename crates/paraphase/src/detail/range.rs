    ...
    > Range<T>
{
    ...
    ///
    ///```
    /// let r = paraphase::detail::range::Range::<i64>::new(0, 10);
    /// assert!(!r.is_empty());
    /// assert_eq!(r.len(), 10);
    /// let r = paraphase::detail::range::Range::<i64>::new(0, 0);
    /// assert!(r.is_empty());
    /// assert_eq!(r.len(), 0);
    /// ```
    ///
    /// # Panics
    /// Panics if end < start, as this cannot be stored in a `usize`.
    #[must_use]
    #[allow(clippy::len_without_is_empty)]
    pub fn len(&self) -> usize {
        (self.end - self.start).try_into().unwrap_or_else(|_| {
            panic!("Type T difference could not be converted to usize. Range: {self:?}")
        })
    }
}
    ...
    for Range<T>
{
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some((self.inner.start, self.inner.end).cmp(&(other.inner.start, other.inner.end)))
    }
}
