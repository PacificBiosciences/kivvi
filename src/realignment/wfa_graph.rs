...
impl WFAGraph {
    ...
    pub fn from_reference_variants(
        reference: &[u8],
        variants: &[Variant],
        ref_start: usize,
        ref_end: usize,
    ...
    /// # Errors
    /// * if there are any errors from adding a node to the graph
    pub fn from_reference_variants_with_hom(
        reference: &[u8],
        variants: &[Variant],
        hom_variants: &[Variant],
        ref_start: usize,
        ref_end: usize,
        max_edit_distance: usize,
    ) -> Result<(WFAGraph, NodeAlleleMap), Box<dyn std::error::Error>> {
        ...
        let mut all_variants: Vec<(&Variant, Option<usize>)> =
            Vec::with_capacity(variants.len() + hom_variants.len());
        ...
        for (variant, variant_index) in all_variants.iter() {
            ...
            if variant_pos < ref_start {
                ...
                trace!(
                    "Ignoring variant starting at {} before ref_start {}",
                    variant_pos,
                    ref_start
                );
                continue;
            ...
            if variant_pos + ref_len > ref_end {
                ...
                trace!(
                    "Ignoring variant ending at {} after ref_end {}",
                    variant_pos + ref_len,
                    ref_end
                );
                continue;
            ...
            while (reconnect_queue
                .peek()
                .unwrap_or((&usize::MAX, &Reverse(usize::MAX)))
                .1)
                .0
                <= variant_pos
            {
    ...
    /// # Errors
    /// * if the maximum edit distance is reached; this is a safeguard from run-away loops
    pub fn edit_distance_with_pruning(
        &self,
        other_sequence: &[u8],
        prune_distance: usize,
    ) -> Result<WFAResult, WFAGraphError> {
        ...
        let mut active_wavefronts: HashMap<usize, HashMap<isize, Vec<(usize, usize)>>> =
            Default::default();
        let mut next_wavefronts: HashMap<usize, HashMap<isize, Vec<(usize, usize)>>> =
            Default::default();
        ...
        loop {
            ...
            for (node_index, node) in self.nodes.iter().enumerate() {
                ...
                if let std::collections::hash_map::Entry::Vacant(e) =
                    encountered_nodes.entry(node_index)
                {
                    e.insert(true);
                    //trace!(
                    //    "WFAGraph n#{} start: min ed = {}",
                    //    node_index,
                    //    edit_distance
                    //);
                }
                ...
                for (other_start, vec_waves) in wavefront.iter_mut() {
                    ...
                    if max_offset < *maxfront_record
                        || (other_start + max_offset as isize) < min_progression as isize
                    {
                        // skips_triggered += 1;
                        // vec_waves.clear();
                        continue;
                    ...
                    if max_offset == node_length {
                        ...
                        if node_index == self.nodes.len() - 1 {
                            ...
                            if ((other_start + max_offset as isize) as usize) < other_sequence.len()
                            {
                                ...
                                let node_wf: &mut HashMap<isize, Vec<(usize, usize)>> =
                                    next_wavefronts.entry(node_index).or_default();
                                ...
                                let plus_diagonal: &mut Vec<(usize, usize)> =
                                    node_wf.entry(*other_start + 1).or_default();
                            ...
                            for &successor_index in self.edges[node_index].iter() {
                                let node_wf: &mut HashMap<isize, Vec<(usize, usize)>> =
                                    active_wavefronts.entry(successor_index).or_default();
                                let copy_diagonal: &mut Vec<(usize, usize)> =
                                    node_wf.entry(new_offset).or_default();

                                // the successor set should include the best + the successors node index
                                let current_set: &BitVec = &index_to_treeset[best_set];
                                let mut new_set: BitVec =
                                    BitVec::from_elem(self.nodes.len(), false);
                                ...
                                let new_set_index: usize = match treeset_to_index.get(&new_set) {
                                    ...
                                    None => {
                                        ...
                                        treeset_to_index
                                            .insert(new_set, index_to_treeset.len() - 1);
                                        index_to_treeset.len() - 1
                                    }
                                };
                        ...
                        let minus_diagonal: &mut Vec<(usize, usize)> =
                            node_wf.entry(*other_start - 1).or_default();
                        ...
                        if ((*other_start + max_offset as isize) as usize) < other_sequence.len() {
                            ...
                            let zero_diagonal: &mut Vec<(usize, usize)> =
                                node_wf.entry(*other_start).or_default();
                ...
                if node_index == self.nodes.len() - 1 {
                    ...
                    if !final_hashsets.is_empty() {
                        ...
                        let sorted_traversed_nodes: Vec<usize> = index_to_treeset[best_set]
                            .iter()
                            .enumerate()
                            .filter(|(_i, b)| *b)
                            .map(|(i, _b)| i)
            ...
            */
            ...
            if edit_distance > self.max_edit_distance {
                ...
                return Err(WFAGraphError::MaxEditDistance {
...
mod tests {
    ...
    fn test_single_node() {
        ...
        assert_eq!(
            graph.edit_distance(&v1).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0]
            }
        );
    ...
    fn test_two_node_single_path() {
        ...
        for split_point in 0..v1.len() {
            ...
            assert_eq!(
                ...
                WFAResult {
                    score: 0,
                    traversed_nodes: vec![0, 1]
                }
            );
            assert_eq!(
                graph.edit_distance(&v2).unwrap(),
                WFAResult {
                    score: 1,
                    traversed_nodes: vec![0, 1]
                }
    ...
    fn test_basic_variant() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 5,
                traversed_nodes: vec![0, 1, 2, 3]
            }
        );
        assert_eq!(
            graph.edit_distance(&v5).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 3]
            }
    ...
    fn test_triple_split() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s1, tail]
            }
        );
        assert_eq!(
            graph.edit_distance(&v2).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s2, tail]
            }
        );
        assert_eq!(
            graph.edit_distance(&v3).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s3, tail]
            }
        );
    }
    ...
    fn test_nested_split() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s2, s3, tail]
            }
        );
        assert_eq!(
            graph.edit_distance(&v3).unwrap(),
    ...
    fn test_double_split() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s1, s2, s3, tail]
            }
        );
        assert_eq!(
            graph.edit_distance(&v2).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s1, s2, s4, tail]
            }
        );
        assert_eq!(
            graph.edit_distance(&v3).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![root, s2, s4, tail]
            }
        );
    }
    ...
    fn test_simple_snv() {
        ...
        let (graph, node_to_alleles): (WFAGraph, NodeAlleleMap) =
            WFAGraph::from_reference_variants(&reference, &variants, 0, reference.len(), 1000)
                .unwrap();
        ...
        assert_eq!(
            graph.edit_distance(&reference).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3]
            }
        ...
        assert_eq!(
            graph.edit_distance("ACA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 3]
            }
        ...
        assert_eq!(
            graph.edit_distance("AA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 3]
            }
    ...
    fn test_multiple_variants() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 5, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("ACAAA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 3, 5, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("AAACA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 4, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("ACACA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 3, 4, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("AAA".as_bytes()).unwrap(),
            WFAResult {
                score: 2,
                traversed_nodes: vec![0, 1, 2, 3, 4, 5, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("AGAGA".as_bytes()).unwrap(),
            WFAResult {
                score: 2,
                traversed_nodes: vec![0, 1, 2, 3, 4, 5, 6]
            }
        ...
        assert_eq!(
            graph.edit_distance("GAAAA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 2, 3, 5, 6]
            }
        ...
        assert_eq!(
            graph.edit_distance("ACAGAA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 3, 5, 6]
            }
    ...
    fn test_overlapping_variants() {
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 4, 5, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("ACTA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 5, 6]
            }
        );
        assert_eq!(
            graph.edit_distance("ACGA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 6]
            }
        );
        // ACGTA -> AGTA, 1 ed; ACTA -> AGTA, 1 ed; so first allele is ambiguous, other should be reference
        assert_eq!(
            graph.edit_distance("AGTA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 4, 5, 6]
            }
        );
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 2,
                traversed_nodes: vec![0, 1, 2, 3, 5, 6]
            }
        );
    ...
    fn test_identical_insertions() {
        ...
        */
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 3, 4]
            }
        );
        assert_eq!(
            graph.edit_distance("ACGTTA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 2, 4]
            }
        );
        // tests an insertion of the wrong character, this should be fully ambiguous
        assert_eq!(
            graph.edit_distance("ACGATA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 3, 4]
            }
        );
        ...
        assert_eq!(
            *node_to_alleles.get(&3).unwrap_or(&vec![]),
            vec![(0, 0), (1, 0)]
        ); // this one has BOTH reference alleles
    ...
    fn test_multiallelic_indel() {
        ...
        */
        ...
        assert_eq!(
            ...
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 4]
            }
        );
        // we can't really get an ambiguous allele assignment here, but we can do ambiguous branches
        assert_eq!(
            graph.edit_distance("ACGGA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 3, 4]
            }
        );
        assert_eq!(
            graph.edit_distance("ACGGTA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 2, 3, 4]
            }
        );
    ...
    fn test_partial_reference() {
        ...
        let (graph, node_to_alleles): (WFAGraph, NodeAlleleMap) =
            WFAGraph::from_reference_variants(&reference, &variants, 2, reference.len() - 2, 1000)
                .unwrap();
        ...
        // remember ALT alleles get added before reference alleles
        assert_eq!(
            graph.edit_distance(&reference[2..5]).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3]
            }
    ...
    fn test_complex_problem() {
        ...
        let (graph, node_to_alleles): (WFAGraph, NodeAlleleMap) =
            WFAGraph::from_reference_variants(&reference, &variants, 2, reference.len() - 1, 1000)
                .unwrap();
        ...
        */
        // remember ALT alleles get added before reference alleles
        assert_eq!(
            graph.edit_distance("CGTTGACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 4, 7, 8]
            }
        ); //reference
        assert_eq!(
            graph.edit_distance("CGACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 8]
            }
        ); //first variant only
        assert_eq!(
            graph.edit_distance("CGTGACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 7, 8]
            }
        ); //second variant only
        assert_eq!(
            graph.edit_distance("CGTTAACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 4, 5, 8]
            }
        ); //third-0 variant only
        assert_eq!(
            graph.edit_distance("CGTTCACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 4, 6, 8]
            }
        ); //third-1 variant only
        assert_eq!(
            graph.edit_distance("CGTAACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 5, 8]
            }
        ); //second and third-0 variants
        assert_eq!(
            graph.edit_distance("CGTCACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 6, 8]
            }
        ); //second and third-1 variants

        // now try some inexacts
        assert_eq!(
            graph.edit_distance("CGGACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 3, 7, 8]
            }
        ); //delete both Ts, ambiguous deletion call
        assert_eq!(
            graph.edit_distance("CGTACGTC".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 2, 3, 5, 6, 7, 8]
            }
        ); //delete TG, ambiguous in a lot of ways
    ...
    fn test_hom_variants() {
        ...
        // remember ALT alleles get added before reference alleles
        assert_eq!(
            graph.edit_distance("AAAAA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 2, 3, 5, 6]
            }
        ...
        assert_eq!(
            graph.edit_distance("ACAA".as_bytes()).unwrap(),
            WFAResult {
                score: 1,
                traversed_nodes: vec![0, 1, 3, 4, 5, 6]
            }
        );
    ...
    fn test_variant_at_start() {
        ...
        let variants =
            vec![
                Variant::new_snv(0, 0, "A".as_bytes().to_vec(), "C".as_bytes().to_vec(), 0, 1)
                    .unwrap(),
            ];
        ...
        assert_eq!(
            graph.edit_distance("CAA".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 3]
            }
    ...
    fn test_variant_at_end() {
        ...
        let variants =
            vec![
                Variant::new_snv(0, 2, "A".as_bytes().to_vec(), "C".as_bytes().to_vec(), 0, 1)
                    .unwrap(),
            ];
        ...
        assert_eq!(
            graph.edit_distance("AAC".as_bytes()).unwrap(),
            WFAResult {
                score: 0,
                traversed_nodes: vec![0, 1, 3]
            }
