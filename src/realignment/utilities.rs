...
impl Variant {
    ...
    pub fn new_deletion(
        ...
        if allele0[0] != allele1[0] {
            ...
            */
            trace!(
                "Deletion alleles are unexpected: {position}, {ref_len}, {allele0:?}, {allele1:?}"
            );
        }
        Ok(Variant {
            vcf_index,
    ...
    pub fn new_sv_deletion(
        ...
        if index_allele0 != 0 || index_allele1 != 1 {
            return Err(VariantError::MultiAllelicNotAllowed {
        ...
        if allele0[0] != allele1[0] {
            ...
            */
            trace!(
                "Deletion alleles are unexpected: {}, {}, {:?}, {:?}",
                position,
                ref_len,
                allele0,
                allele1
    ...
    pub fn new_sv_insertion(
        ...
        if allele0[0] != allele1[0] {
            // no counter example searched for yet, but probably exists, we'll leave this trace for now
            trace!(
                "Insertion alleles are unexpected: {}, {}, {:?}, {:?}",
                position,
                1,
                allele0,
                allele1
    ...
    pub fn closest_allele_clip(
        ...
        let d1: usize = edit_distance(
            allele,
            &self.allele1[head_clip..(self.allele1.len() - tail_clip)],
        );
...
mod tests {
    ...
    fn test_edit_error_001() {
        let v1 = [
            65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 67, 65, 65, 65,
        ];
        let v2 = [
            65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 67, 65, 65, 65, 65, 65, 65, 67, 65, 65, 65,
        ];
        let v3 = [
            65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 67, 65, 65, 65,
        ];
    ...
    fn test_basic_deletion() {
        ...
        let variant =
            Variant::new_deletion(0, 10, 3, b"AGT".to_vec(), b"A".to_vec(), 0, 1).unwrap();
        ...
        let variant = Variant::new_deletion(0, 10, 4, b"C".to_vec(), b"A".to_vec(), 1, 2).unwrap();
        assert_eq!(variant.get_type(), VariantType::Deletion);
        assert_eq!(variant.position(), 10);
    ...
    fn test_basic_insertion() {
        let variant = Variant::new_insertion(0, 20, b"A".to_vec(), b"AGT".to_vec(), 0, 1).unwrap();
    ...
    fn test_sv_insertion() {
        let variant =
            Variant::new_sv_insertion(0, 20, 1, b"A".to_vec(), b"AGT".to_vec(), 0, 1).unwrap();
    ...
    fn test_sv_deletion() {
        let variant =
            Variant::new_sv_deletion(0, 10, 3, b"AGT".to_vec(), b"A".to_vec(), 0, 1).unwrap();
    ...
    fn test_tandem_repeat() {
        let variant =
            Variant::new_tandem_repeat(0, 10, 4, b"AAAC".to_vec(), b"AAACAAAC".to_vec(), 0, 1)
                .unwrap();
